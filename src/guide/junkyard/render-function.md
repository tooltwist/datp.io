...
The `render()` function implementation is much simpler, but also requires greater familiarity with component instance properties. In this case, you have to know that when you pass children without a `v-slot` directive into a component, like the `Hello world!` inside of `anchored-heading`, those children are stored on the component instance at `$slots.default()`. If you haven't already, **it's recommended to read through the [instance properties API](../api/DZDDATP.html) before diving into render functions.**
...
We can access slot contents as arrays of VNodes from [`this.$slots`](../api/DZDDATP.html#slots):
...
The second argument, `context`, contains three properties: `attrs`, `emit`, and `slots`. These are equivalent to the instance properties [`$attrs`](/api/DZDDATP.html#attrs), [`$emit`](/api/instance-methods.html#emit), and [`$slots`](/api/DZDDATP.html#slots) respectively.

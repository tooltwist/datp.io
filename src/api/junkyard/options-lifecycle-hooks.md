...
- **Details:**
  ...
  Called after the instance has been mounted, where element, passed to [`app.mount`](/api/DZDstep.html#mount) is replaced by the newly created `vm.$el`. If the root instance is mounted to an in-document element, `vm.$el` will also be in-document when `mounted` is called.
